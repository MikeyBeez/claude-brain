...
class BudgetManager {
  constructor(totalBudget = 0.30) {
    ...
    console.error('💰 BudgetManager initialized with total budget:', totalBudget);
  }
  ...
  resetForSession(sessionId, options = {}) {
    ...
    console.error(`💰 Budget reset for session ${sessionId}: ${this.budgetState.total} total budget`);
  }
  ...
  setTierBudgets(tierBudgets) {
    ...
    console.error('📊 Tier budgets allocated:', tierBudgets);
  }
  ...
  canAfford(cost, tier = null) {
    ...
    if (this.budgetState.used + cost > this.budgetState.total - this.budgetState.reservedBudget) {
      console.error(`💸 Cannot afford ${cost}: would exceed total budget`);
      return false;
    }
    ...
    if (tier && this.budgetState.tierAllocations[tier]) {
      ...
      if (tierUsed + cost > tierBudget) {
        console.error(`💸 Cannot afford ${cost} in ${tier}: would exceed tier budget (${tierUsed + cost} > ${tierBudget})`);
        return false;
      }
    }
  ...
  recordUsage(actualCost, tier = null, metadata = {}) {
    ...
    console.error(`💰 Recorded usage: ${actualCost} (${tier}) | Total: ${this.budgetState.used.toFixed(3)} | Remaining: ${this.budgetState.remaining.toFixed(3)}`);
  }
  ...
  optimizeBudgetAllocation(usagePatterns = {}) {
    console.error('🎯 Optimizing budget allocation based on usage patterns...');
