#!/usr/bin/env node
// Phase 1: Connect orphaned notes to their obvious hubs
// This script adds [[Hub Name]] links to orphaned notes based on naming patterns
...
const connectionRules = [
  ...
  {
    pattern: /^medical[_\s-]|health[_\s-]/i,
  ...
  {
    pattern: /philosophy|consciousness|theory|dimensional/i,
...
// First, check if hub notes exist or need to be created
const hubsToCreate = [
...
for (const noteName of orphanedNotes) {
  ...
  if (matchingRule) {
    const notePath = path.join(VAULT_PATH, `${noteName}.md`);
    ...
    if (fs.existsSync(notePath)) {
      try {
        ...
        if (!content.includes(`[[${matchingRule.hub}]]`)) {
          ...
          connectionsAdded++;
          processedNotes.push({ note: noteName, hub: matchingRule.hub });
        } else {
...
console.log(`Orphans remaining unprocessed: ${orphanedNotes.length - processedNotes.length - skippedNotes.length - notFoundNotes.length}`);
...
// Save a report
const reportPath = '/Users/bard/Code/claude-brain/phase1-connections-report.json';
fs.writeFileSync(reportPath, JSON.stringify({
  ...
  skippedNotes,
  notFoundNotes,
  orphansRemaining: orphanedNotes.filter(n => !processedNotes.find(p => p.note === n) && !skippedNotes.includes(n) && !notFoundNotes.includes(n)),
  byHub
}, null, 2));
